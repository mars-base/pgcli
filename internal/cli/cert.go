...
func init() {
	...
	certCmd.Flags().BoolVar(&certCA, "ca", false, "issue a self-signed CA (CA:TRUE, keyCertSign) instead of a server leaf — do not feed this to a MinIO/silo/rustfs --tls-cert")
...
is also the pair MinIO's, silo's, and rustfs's bring-your-own-TLS mode consumes
directly (rustfs reads it under the names rustfs_cert.pem/rustfs_key.pem, but
pgcli mounts your files at those names, so any filenames work):
  ...
  pg addon install rustfs --name store --tls-cert minio.crt --tls-key minio.key
