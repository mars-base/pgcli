...
func init() {
	...
	certCmd.Flags().BoolVar(&certCA, "ca", false, "issue a self-signed CA (CA:TRUE, keyCertSign) instead of a server leaf — do not feed this to MinIO --tls-cert")
...
is also the pair MinIO's bring-your-own-TLS mode consumes directly:
