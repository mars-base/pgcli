---
title: "S3 存储高可用方案"
description: "让 pgBackRest 背后的 MinIO/silo 对象存储也高可用：pgcli 暴露的三种部署形态（SNSD/SNMD/MNSD）、原生 SNMD 与 ZFS 磁盘冗余层的取舍，以及分布式集群每节点各自建池的 ZFS 配方"
weight: 55
---
...
| 故障 | 由谁吸收 |
|------|----------|
| 一个节点内的一块**磁盘**坏了 | SNMD 下的盘级 EC，或 MinIO 数据目录底下的存储层（ZFS） |
...
（[SNSD / SNMD / MNSD](../addon/minio/#部署形态)）。pgcli 暴露其中三种：
...
SNMD 现已原生支持：`pg addon install minio --drive /mnt/disk1 --drive ...`
...
"多机且每台多盘"（MNMD）仍未做进插件：`--endpoint` 每节点只收一个可路由主机，
所以分布式集群仍保持每节点一块盘的形态。
...
| | 原生 **SNMD**（`--drive`） | `--data-dir` 底下的 **ZFS** 池 |
|---|---|---|
...
| 服务哪些形态 | 仅 SNMD | SNSD *与* MNSD（同一配方垫在每个节点下） |
| 异构节点 | 每个成员都得是一组盘 | 2 盘节点与 6 盘节点看起来一样（各一个 endpoint） |

坦白的取舍是：**SNMD 更简单**——一条命令、不用置备文件系统、零 ZFS 配置就在
单机拿到磁盘冗余。**ZFS 更灵活**——底层布局随时可换、同盘下省出更多可用容量
（`raidz1` 4 盘留 3 份，而 SNMD 的 EC:2 只留 2 份），而且它是*唯一*能同时服务
MNSD 的路——那里坏盘绝不能升级成丢节点。于是：

- 单机、要磁盘冗余又不想碰 ZFS → **SNMD**
- 单机、要更多可用容量 / 日后还想换布局 → **SNSD + ZFS**
- 分布式集群、盘和节点都要扛 → **MNSD + 每节点 ZFS**（SNMD 跨不了主机，那正是
  pgcli 暂未暴露的 MNMD 形态）

单机上两条路都不算错——SNMD 那 50% 容量是不用管理池子的代价，ZFS 的灵活是置
备池子的代价。下文继续讲 ZFS 这条路：对 MNSD 和容量敏感的单机主机，它仍是推
荐。
...
| 每台主机多块数据盘，盘和主机都要扛得住 | MNSD + 每节点 ZFS（各节点布局可不同；容量对齐）——SNMD 跨不了主机 |
