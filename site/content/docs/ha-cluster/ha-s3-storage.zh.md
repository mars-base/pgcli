...
缺掉的那一种是 **SNMD**（单机*多*盘）——这不是取舍，而是 MinIO 本身拒绝：
一个分布式 endpoint 列表若全部解析到同一主机，会直接被拒。

- 同主机列表报 `use path style endpoint for single node setup`；
- 整个 `127.0.0.0/8` 回环段报 `resolves to localhost`。

也就是说 SNMD 那个容器根本起不来——上游明令禁止的部署形态，pgcli 不可能提
供。SNMD 本该提供的能力（一块主机上的多块盘组成一个有冗余的卷）被下放到更
低一层，交给文件系统：**ZFS 把任意数量的物理盘，包装成 MinIO 看到的"单块可
靠盘"。**分工因此很干净——MinIO 负责节点级 EC，ZFS 负责磁盘级冗余——每一层
都保持简单。
